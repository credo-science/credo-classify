...
from hit_analysis.commons.consts import CLASSIFIED, CLASS_ARTIFACT, X, Y, ARTIFACT_NEAR_HOT_PIXEL
...
def near_hot_pixel_classify(group: Dict[Tuple[int, int], List[dict]], often: int) -> None:
    """
...
def near_hot_pixel_process(groups: Dict[Tuple[int, int], Dict[Tuple[int, int], List[dict]]], often: int = 3) -> None:
    ...
    for k, v in groups.items():
        near_hot_pixel_classify(v, often)
